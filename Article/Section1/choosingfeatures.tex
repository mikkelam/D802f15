...
Throughout this article, we will by $T = \{0, 1\}$ denote the two teams \textit{blue} and \textit{red}, respectively, and by $C = \{0, 1, \dots, m-1\}$ denote the set of all champions. When referring to features, we will by $t$ denote an element in $T$ and by $c$ denote an element in $C$.

Some champions may be better than other champions.
...
Some champions, considered damage dealers, deal very high damage, but die very easily when taking damage. Other champions deal very little damage, but can be almost impossible to kill these are considered tanks. When alone, these two types of champions are not that strong, but when teaming up, they can pose a serious threat. The tank can be used by the damage dealer as a living shield, allowing him to stay alive for much longer, thus deal more damage.
