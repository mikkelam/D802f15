...
def team_composition(line):
  ...
  featureList = [0] * 249 # a list of zeros. index 0 is the label/class, followed by the featurelist
  for p in json_object[ "participants" ]:
    champIndex = 1 # index 0 must be the label
    champIndex += __champion_index(p[ "championId" ])
    if p[ "teamId" ] == 200:
      champIndex += 124 # first 123 is team blue, following 123 is team red
