...
The linear regression model generates a prediction $\hat{y}$ of the value $y$. $\hat{y}$ is found with the hypothesis function $h(x)$ by assigning weights $(\mathbf{w}_1\phi_1),(\mathbf{w}_2\phi_2), \dots (\mathbf{w}_M\phi_M)$ to the features and computing their sum.
...
Since the goal is to fit the hypothesis function to the training set, linear regression uses a loss function to indicate how well the hypothesis function fits.
Given the data $D$ and the weights $w$, the loss function is defined as
