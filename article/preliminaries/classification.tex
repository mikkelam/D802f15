...
$$\hat{y} = \begin{cases}
	1 &\text{ if } f(x) > 0.5 \\
	0 &\text{ if } f(x) \leq 0.5  % should this be 0 < f(x) \leq 0 ????
\end{cases}$$ 
...
By doing this a single outlying in the training data is no longer able to skew the decision boundary of the hypothesis function.
