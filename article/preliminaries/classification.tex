...
The linear regression model predicts the value of $y$ with $\hat{y}$. $\hat{y}$ is found with the hypothesis function $h(x)$ by assigning weights $(\textbf{w}_1\phi_1),(\textbf{w}_2\phi_2), \text{ ... } (\textbf{w}_M\phi_M)$. to the features and computing their sum.
$$h_w(x) = \hat{y} = w_0 + \sum_{i=1}^M w_i \phi_i(x)$$ 

Where $w_0$ is as special case called the intercept, however to avoid having this special case we add  $w_0$ to the feature $\phi_0(x)=1$, which allows us to use the intercept as a normal feature. 
