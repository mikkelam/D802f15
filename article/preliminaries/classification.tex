...
In \Cref{sec:supervisedclassification} supervised classification is described followed by a more detailed explanation of a particular classification model called logistic regression, which is widely used in this project. In \Cref{sec:distributed}, the distributed file system Hadoop and the large scale data processing engine Apache Spark is introduced, as well as the motivation for using those tools in this project.
...
Because Stochastic Gradient Descent updates the weights using individual datapoints.
The value has a tendency to jump around alot instead of moving directly down the gradient towards the minimum.
