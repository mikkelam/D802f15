...
The examples include a constant, the identity, a polynomiel and the combination of two previously defined features.
...
\[ L_w = \lambda \sum_{j=1}^{M-1} w_j^2 \]

Where the regularisation constant $\lambda$ scales the penalty. 
For Logistic regression the combined loss function looks like this.
...
\[ L(\textbf{w})
  = L_D(w) + L_w 
  = \sum_{n=1}^N E_w(y_n, h_w(x_n)) + \lambda \sum_{j=1}^{M-1} w_j^2 \] 
...
\todo{TODO: WRITE ABOUT L1 REGULARIZATION.}
...
In batch gradient descent we iteratively update the weights by a value proportinal to the partial derivative of the loss function.  
...
When dealing with large dataset this method runs into a problem, namely that computing the partial derivative requires summing over all of the datapoints in the training set. 
