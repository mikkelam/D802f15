...
\begin{figure}[!htb]
  ...
  \begin{tikzpicture}[] 
    \begin{axis}[
      ...
      \legend{$\phi_\text{LANE-CHAMPION}$,$\phi_\text{SINGLE}$,$\phi_\text{LANE-RANKS}$,$\phi_\text{COUNTERS}$,$\phi_\text{BEST-RANKED}$,$\phi_\text{PAIRS}$,$\phi_\text{PLAYER-RANKS}$,No features}
    \end{axis} 
  \end{tikzpicture}
