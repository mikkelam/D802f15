...
In this section, the cluster that will be doing the computations will be outlined. In \Cref{sec:clustersetup} the setup of the cluster and the available resources are listed, followed by a profile of the resource usage in \Cref{sec:profile}. In \Cref{sec:benchmark} the performance of the cluster will be tested against a a single worker node, both doing word count as a measurement. In \Cref{sec:speedup} the speedup from using additional nodes for computations will be measured.
\todo{hvis 4.3 og 4.4 slåes sammen}
